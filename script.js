...
const translations = {
    ar: {
        ...
        toggleLighting: 'تفعيل الإضاءة',
        toggleKelvin: 'تعطيل تأثير كلفن',
        // toggleFlatColors removed; flat colours feature no longer available
    },
    en: {
        ...
        toggleLighting: 'Enable Lighting',
        toggleKelvin: 'Disable Kelvin effect',
        // toggleFlatColors removed; flat colours feature no longer available
    }
};

// Current language (default Arabic). Check localStorage for saved preference
...
function updateLanguageUI() {
    ...
    // Lighting toggle label (if present)
    const lightingToggleLabel = document.getElementById('label-toggle-lighting');
    if (lightingToggleLabel) lightingToggleLabel.textContent = translations[currentLanguage].toggleLighting;
    // Kelvin effect toggle label
    const kelvinToggleLabel = document.getElementById('label-toggle-kelvin');
    if (kelvinToggleLabel) kelvinToggleLabel.textContent = translations[currentLanguage].toggleKelvin;
    // Flat colours toggle removed; no label to update
    ...
    // Update lighting text and appearance depending on Kelvin effect state
    if (document.getElementById('current-lighting')) {
        if (kelvinEffectEnabled) {
    ...
    // Update the HTML and body direction based on language
    document.documentElement.lang = currentLanguage;
    const dirValue = (currentLanguage === 'ar') ? 'rtl' : 'ltr';
...
function kelvinToRGB(kelvin) {
    // Constrain kelvin to a reasonable range
    ...
    if (temp <= 66) {
        // For temperatures below 6600K, the red channel is always at maximum
        red = 255;
        // Green channel increases logarithmically
        green = 99.4708025861 * Math.log(temp) - 161.1195681661;
        green = Math.max(0, Math.min(255, green));
        // Blue channel is 0 below 1900K and increases logarithmically after
        if (temp <= 19) {
            blue = 0;
        } else {
        ...
        // For temperatures above 6600K, the blue channel is always at maximum
        blue = 255;
        // Red channel decreases according to a power law
        red = 329.698727446 * Math.pow(temp - 60, -0.1332047592);
        red = Math.max(0, Math.min(255, red));
        // Green channel decreases according to a power law
        green = 288.1221695283 * Math.pow(temp - 60, -0.0755148492);
        green = Math.max(0, Math.min(255, green));
    }

    // Normalize to 0-1 range for Three.js colors
    return {
        r: red / 255,
        g: green / 255,
        b: blue / 255
...
// Determines whether Kelvin lighting effect is enabled (true) or disabled (false).
// Determines whether Kelvin lighting effect is enabled (true) or disabled (false).
// Default is false so no Kelvin light is selected initially.
let kelvinEffectEnabled = false;
// Removed flatColours flag as feature has been discontinued
let furnitureState = {
    bed: true,
    wardrobe: true,
    dresser: false,
...
function init() {
    ...
    // Do not apply physically correct lighting or tone mapping; this ensures raw colours appear without post-processing
    renderer.physicallyCorrectLights = false;
    // Use linear colour encoding so colours are not converted; this keeps the raw hex values unchanged
    renderer.outputEncoding = THREE.LinearEncoding;
    // Disable tone mapping so colours are not adjusted
    renderer.toneMapping = THREE.NoToneMapping;
    renderer.toneMappingExposure = 1.0;
    // Disable shadows to avoid any shading or shadow effects on colours
    ...
    // Initialize lighting system: use neutral lighting when Kelvin effect is disabled
    if (kelvinEffectEnabled) {
        setupLighting(currentLighting);
    } else {
        setupNeutralLighting();
...
function setupLighting(kelvin) {
    ...
    const color = new THREE.Color(rgb.r, rgb.g, rgb.b);
            ...
            // Adjust intensity based on temperature for more realistic indoor lighting
            let ambientIntensity, directionalIntensity, pointIntensity;

            if (kelvin <= 3500) {
                // Warm lights: slightly higher ambient for a cosy feel
                ambientIntensity = 0.65;
                directionalIntensity = 0.35;
                pointIntensity = 0.25;
            } else if (kelvin <= 6000) {
                // Neutral lights: balanced distribution
                ambientIntensity = 0.6;
                directionalIntensity = 0.4;
                pointIntensity = 0.3;
            } else {
                // Cool lights: cooler tones but still balanced lighting
    ...
    // Directional light from above
            ...
            // Enable shadow casting for more realistic shadows
            directionalLight.castShadow = true;
            // Configure shadow map size for smoother shadows
            directionalLight.shadow.mapSize.width = 2048;
            directionalLight.shadow.mapSize.height = 2048;
            // Configure shadow camera to cover the room (adjust as needed)
            const d = 10;
            ...
            // Slight bias to reduce shadow artifacts
...
// Disable lighting by removing all lights from the scene
function disableLighting() {
    Object.values(lights).forEach(light => {
        if (light) scene.remove(light);
    });
    lights = {};
    // Optionally update UI to indicate lighting is disabled
    const lightingEl = document.getElementById('current-lighting');
    if (lightingEl) {
        lightingEl.textContent = translations[currentLanguage].currentLighting + ' —';
    }
    // Clear active state from lighting buttons
    document.querySelectorAll('.lighting-btn').forEach(btn => {
        btn.classList.remove('active');
    });
}

// Set up neutral lighting (pure white light) to display wall colors without Kelvin effect
function setupNeutralLighting() {
    ...
    // To display the selected colours without any Kelvin tint or directional shading,
    // add only a white ambient light at full intensity. This ensures that all surfaces
    // display their true colours uniformly, unaffected by the temperature-based lights.
    const whiteAmbient = new THREE.AmbientLight(new THREE.Color(1, 1, 1), 1.0);
    scene.add(whiteAmbient);
    lights.ambient = whiteAmbient;

    // Update UI: show dash (no Kelvin value) to indicate Kelvin effect is disabled
    const lightingEl = document.getElementById('current-lighting');
    if (lightingEl) {
        lightingEl.textContent = `${translations[currentLanguage].currentLighting} —`;
    }

    // Remove active state from lighting buttons since Kelvin selection is inactive
    document.querySelectorAll('.lighting-btn').forEach(btn => {
        btn.classList.remove('active');
    });
}

// Enable or disable flat colours (no shading). When enabled, all objects use MeshBasicMaterial and
// no lights are present. When disabled, original materials are restored and lighting is re-applied.
// Removed enableFlatColors function as flat colours feature is no longer supported
...
function addFurniture(width, length, height) {
    ...
    // After all furniture objects are added, configure shadow properties
    Object.values(furnitureObjects).forEach(obj => {
        if (!obj) return;
        // Carpets should receive shadows but not cast them
        if (obj.userData && obj.userData.type === 'carpet') {
            obj.receiveShadow = true;
            obj.castShadow = false;
        } else {
            // Other furniture casts shadows but does not receive them
            obj.castShadow = true;
...
function updateRoom() {
    ...
    wallInputs.forEach((wallId, index) => {
        ...
        if (colorInfo.hex) {
            ...
            const wallGeometry = new THREE.PlaneGeometry(config.width, config.height);
                // Use MeshBasicMaterial so selected wall colour displays as raw (unlit) colour without lighting effects
    ...
    // Set shadow properties: floors and walls receive shadows; other room objects cast shadows
    Object.keys(roomObjects).forEach(key => {
        const obj = roomObjects[key];
        if (!obj) return;
        if (key === 'floor' || key.startsWith('wall') || key === 'ceiling') {
            obj.receiveShadow = true;
        } else {
            obj.castShadow = true;
        }
    ...
    // Flat colours mode removed: no need to reapply materials
}
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (langSelector) {
        // Set initial value based on stored or current language
        langSelector.value = currentLanguage;
        langSelector.addEventListener('change', function () {
            // Update language and persist preference
    ...
    document.querySelectorAll('.lighting-btn').forEach(btn => {
        btn.addEventListener('click', function () {
            ...
            // If Kelvin effect is currently enabled and the clicked button corresponds
            // to the current Kelvin temperature, treat this as a toggle off action.
            if (kelvinEffectEnabled && currentLighting === temp) {
                kelvinEffectEnabled = false;
                setupNeutralLighting();
            } else {
                // Otherwise enable Kelvin effect with the selected temperature
                kelvinEffectEnabled = true;
    ...
    // Kelvin effect toggle checkbox removed; Kelvin buttons themselves toggle the effect.
    ...
    document.getElementById('reset-btn').addEventListener('click', function () {
        ...
        // Flat colours mode removed; nothing to reset here
    ...
    // Auto-hide the header on scroll (mobile only)
    let lastScrollTop = 0;
    window.addEventListener('scroll', function () {
        const header = document.querySelector('.header');
        if (!header) return;
        const viewportWidth = window.innerWidth || document.documentElement.clientWidth;
        // Only apply hide/show behaviour on mobile widths (<= 768px)
